...
_EASE_KEYWORD_RULES: tuple[dict[str, Any], ...] = (
    {
        "pattern": re.compile(
    ...
    {
        "pattern": re.compile(
            r"\b(members only|nur fur mitglieder|only for members|nur fur vis mitglieder|nur fur vmp mitglieder)\b"
        ),
        "weight": -0.4,
        "label": "Members only",
    },
...
_EASE_POSITIVE_GUARDS: tuple[str, ...] = (
    "no registration required",
    "no registration needed",
    "keine anmeldung erforderlich",
    "keine anmeldung notig",
    "ohne anmeldung",
)

_EASE_REMAINING_PLACES_PATTERN = re.compile(
...
def infer_ease_of_entry(corpus: str, *, price: Any = None, spots: Any = None) -> dict[str, Any]:
    """
    Estimate how difficult it is to access an event based on textual cues.

    The function consumes an already concatenated text corpus, applies a small
    rule set with weighted keywords and returns both the computed score and the
    matched signals.  Scores are normalised to the ``0..1`` interval where
    values close to ``1`` indicate very easy entry.  When no clue is found the
    function returns ``None`` for the score so callers can supply their own
    fallback.

    Parameters
    ----------
    corpus:
        Pre-processed event description that should already be lower-case and
        accent stripped.  Passing the raw crawl content works as well because
        the function normalises it internally.
    price:
        Optional structured price information (e.g. numeric CHF value) that was
        already parsed from the raw payload.
    spots:
        Optional structured number of remaining spots captured in the payload.

    Returns
    -------
    dict
        Dictionary with ``score`` (``float`` in ``0..1`` or ``None`` when no
        signals were detected) and ``signals`` (``list[str]``) describing the
        matched heuristics for transparency.
    """
    ...
    # We currently only find negative keywords, so start from 1.0 and deduct.
    score = 1.0
    signals: list[str] = []
    ...
    for rule in _EASE_KEYWORD_RULES:
        ...
        if weight < 0 and label == "Registration required":
            if any(guard in text for guard in _EASE_POSITIVE_GUARDS):
                continue

        score += weight
        signals.append(label)

    match = _EASE_REMAINING_PLACES_PATTERN.search(text)
    if match:
        try:
    ...
    price_value = _coerce_number(price)
    if price_value is not None:
        if price_value <= 0:
            score += 0.25
            signals.append("Structured price indicates free entry")
        else:
            score -= 0.35
            signals.append("Structured price indicates a fee")

    spots_value = _coerce_number(spots)
    if spots_value is not None:
        if spots_value <= 0:
            score -= 0.4
            signals.append("Structured spots: no availability")
        elif spots_value <= 3:
            score -= 0.2
            signals.append("Structured spots: very limited")
        elif spots_value <= 10:
            score -= 0.05
            signals.append("Structured spots: limited")
        elif spots_value >= 25:
            score += 0.15
            signals.append("Structured spots: plenty available")

    if not signals:
        return {"score": None, "signals": []}

    bounded = max(0.0, min(1.0, score))
    return {"score": bounded, "signals": signals}
